/* Singleton pattern. Connects to database on first use of .open() 
    and subsequent calls return the same connection object. 

The .open() method returns the db object.

Using this API means that if connections time out, we can replace this code 
    with something that connects every time.
